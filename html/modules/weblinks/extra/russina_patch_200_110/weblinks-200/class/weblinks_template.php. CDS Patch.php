<?php
// $Id: weblinks_template.php.\040CDS\040Patch.php,v 1.1 2012/04/09 10:21:09 ohwada Exp $
...
// config
	...
	var $_total_site_gmap;
	var $_total_site_rss;
	var $_total_feed;

// keyword
	var $_keyword_array      = array();
	var $_keywords_urlencode = null;
	var $_keyword_query      = null;
...
/* CDS Patch. Weblinks. 2.00. 6. BOF */
function fetch_guidance_rssicon()
{
	$tpl =& $this->_create_xoops_tpl();
	...
	$text = $tpl->fetch( $this->_get_parts_file( 'weblinks_guidance_rssicon.html' ) );
	return $text;
}
/* CDS Patch. Weblinks. 2.00. 6. EOF */
...
function fetch_links_full_list(&$links)
{
	$ret = array();
	foreach ($links as $link)
	{
		$ret[] = $this->fetch_link_single($link);
	}
	return $ret;
}
...
function fetch_d3forum_comment( $id, $subject )
{
	$text = '';
...
/* CDS Patch. Weblinks. 2.00. 7. BOF */
	$tpl->assign('total_site_topten_number', intval($this->_conf['topten_links']));
...
// config_handler
//---------------------------------------------------------
function _assign_config_name( &$tpl )
{
...
function set_keyword_query( $val )
{
...
// template parts file
//---------------------------------------------------------
function _get_parts_file( $name )
{
	return $this->_get_custom_file( 'parts', $name ) ;
}

function _get_custom_file( $dir, $name )
{
/* CDS Patch. Weblinks. 2.00. 1. BOF */
	global $xoopsConfig;
	$file_theme = XOOPS_THEME_PATH.'/'.$xoopsConfig['theme_set'] .'/modules/' . $this->_DIRNAME . '/' . $dir . '/' . $name;
	$file_theme_custom = XOOPS_THEME_PATH.'/'.$xoopsConfig['theme_set'] .'/modules/' . $this->_DIRNAME . '/custom/' . $name;
	if (file_exists($file_theme_custom))
	{
		return $file_theme_custom;
	}
	elseif (file_exists($file_theme))
	{
		return $file_theme;
	}
	else
